...
When the user runs `/rs` (or `/rs <topic>`), scan the project state and present a context-aware guide to available skills.

## Arguments

`/rs` -- full navigator with deck state + skill guide
`/rs compose` -- deep help on a specific skill
`/rs "I have markdown, what now?"` -- intent-based routing
...
**If no argument** (bare `/rs`): show the full navigator (Step 3).

**If argument matches a skill name** (e.g., `/rs compose`): read that skill's `.md` file from `.claude/plugins/rs/commands/` or `.claude/commands/` and present a concise usage summary: what it does, arguments, example invocation, and when to use it vs alternatives.
...
|-------|-------------|----------|
| `/compose` | Assign design directives to slides | You want control over composition settings |
| `/build-deck` | Full pipeline with auto-convergence | You want one command, hands-off |
| `/compare` | Try 3-12 variants side by side | You want to explore before committing |

Note: `/compose`, `/build-deck`, and `/compare` are in the `rs` plugin. Invoke as `/rs:compose`, `/rs:build-deck`, `/rs:compare` if the short form doesn't resolve.
